defmodule ExqliteTest do
  ...
  describe ".open/2" do
    ...
    test "opens a database in readonly mode" do
      ...
      assert {:error, %Exqlite.Error{code: 8} = error} =
               Exqlite.execute(
                 conn,
    ...
    test "opens a URL" do
      ...
      assert error.code == 8
      assert error.codename == "attempt to write a readonly database"
  ...
  describe ".execute/2" do
    ...
    test "handles incorrect syntax", %{conn: conn} do
      assert {:error, %Exqlite.Error{code: 1} = error} =
               Exqlite.execute(
                 conn,
                 "create a dumb table test (id integer primary key, stuff text)"
               )

      assert Exception.message(error) == "SQL logic error: near \"a\": syntax error"
  ...
  describe ".prepare/2" do
    ...
    test "users table does not exist", %{conn: conn} do
      ...
      assert error.code == 1
      assert Exception.message(error) == "SQL logic error: no such table: users"
    end
    ...
    test "supports utf8 in error messages", %{conn: conn} do
      ...
      assert Exception.message(error) == "SQL logic error: no such table: 🌍"
    end
  end
  ...
  describe "working with prepared statements after close" do
    test "returns proper error" do
      ...
      assert {:error, %Exqlite.Error{code: 21} = error} =
               Exqlite.execute(
                 conn,
                 "create table test (id integer primary key, stuff text)"
               )
      ...
      assert Exception.message(error) ==
               "bad parameter or other API misuse: out of memory"
